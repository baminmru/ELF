...
finalRet:
    ...
    Public Function writeMessage(ByVal buf() As Byte, ByVal ret As Short) As String
        ...
        Try
            ...
            If (buf(2) = &H52 And IsmArchToRead = True) Then 'чтение мгновенного архива
                ...
                If (tv = 1) Then
                    'mArch.НСtv1 = Asc(Mid(str, Adr, 1)) * 256& ^ 2 + Asc(Mid(str, Adr + 1, 1)) * 256& + Asc(Mid(str, Adr + 2, 1))
                ...
                End If
                If (tv = 2) Then
                    'mArch.НСtv2 = Asc(Mid(str, Adr, 1)) * 256& ^ 2 + Asc(Mid(str, Adr + 1, 1)) * 256& + Asc(Mid(str, Adr + 2, 1))
                    mArch.НСtv2 = Asc(Mid(str, Adr + 2, 1)) * 256& ^ 2 + Asc(Mid(str, Adr + 1, 1)) * 256& + Asc(Mid(str, Adr, 1))
                    mArch.MsgНС = DeCodeHC(mArch.НСtv2)
            ...
            If (buf(2) = &H48) Then 'часовой архив
                ...
                End If
                If (tv = 2) Then
                    'Arch.НСtv2 = Asc(Mid(hourstr, Adr, 1)) * 256& ^ 2 + Asc(Mid(hourstr, Adr + 1, 1)) * 256& + Asc(Mid(hourstr, Adr + 2, 1))
                    Arch.НСtv2 = Asc(Mid(hourstr, Adr + 2, 1)) * 256& ^ 2 + Asc(Mid(hourstr, Adr + 1, 1)) * 256& + Asc(Mid(hourstr, Adr, 1))
                    Arch.MsgНСtv2 = DeCodeHC(Arch.НСtv2)
    ...
    Public Sub EraseInputQueue()
        ...
        i = 1
        While (i > 0 And MyTransport.IsConnected And MyTransport.BytesToRead > 0)
            i = MyTransport.Read(buffer, 0, 1)
        End While
        For i = 0 To 73
            buffer(i) = 0
        Next
...
'''''''''''''''''''''''
 ...
 Public Function NanFormat(ByVal n As Single, ByVal fStr As String) As String
    ...
    Public Overrides Function WriteArchToDB() As String
        ...
        WriteArchToDB = WriteArchToDB + "'" + S180(DeCodeHCText(Arch.НСtv1)) + "',"
        WriteArchToDB = WriteArchToDB + "'" + S180(DeCodeHCText(Arch.НСtv2)) + "'"
    ...
    Public Overrides Function WriteMArchToDB() As String
        ...
        WriteMArchToDB = WriteMArchToDB + "'" + S180(DeCodeHCText(mArch.НСtv1)) + "',"
        WriteMArchToDB = WriteMArchToDB + "'" + S180(DeCodeHCText(mArch.НСtv2)) + "'"
