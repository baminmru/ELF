<?php
	 class C_tplc_h extends CI_Controller {
    function __construct() {
         parent::__construct();
        $this->_loadModels();
    }
    function setRow() {
          log_message('debug', 'TPLC_H.setRow post : '.json_encode($this->input->post(NULL, FALSE)));
          log_message('debug', 'TPLC_H.getRows get : '.json_encode($this->input->get(NULL, FALSE)));
          $data = array(
                'tplc_hid' =>  $this->input->get_post('tplc_hid', TRUE)
            ...
            $tplc_h = $this->m_tplc_h->setRow($data);
            print json_encode($tplc_h);
    }
    function newRow() {
            log_message('debug', 'TPLC_H.newRow post : '.json_encode($this->input->post(NULL, FALSE)));
          $data = array(
                'tplc_hid' =>  $this->input->get_post('tplc_hid', TRUE)
                ,'instanceid' =>  $this->input->get_post('instanceid', TRUE)
                ,'dcall' =>   $this->input->get_post('dcall', TRUE)
                ...
                $instanceid =  $this->input->get_post('instanceid', TRUE);
            $tplc_h= $this->m_tplc_h->newRow($instanceid,$data);
            $return = $tplc_h;
            print json_encode($return);
    }
    function getRow() {
        log_message('debug', 'TPLC_H.getRow post : '.json_encode($this->input->post(NULL, FALSE)));
        $empId  =  $this->input->get_post('id', TRUE);
        if (isset($empId)) {
            $tplc_h = $this->m_tplc_h->getRow($empId);
            $return = array(
                'success' => true,
                'data'    => $tplc_h
            );
    ...
    function getRows() {
           ...
           if(!$sort || $group == $sort) 
            ...
            if(isset($instanceid)){
                if($instanceid!=''){
                    $tplc_h= $this->m_tplc_h->getRowsByInstance($instanceid,$sort);
                }else{
                    $tplc_h= $this->m_tplc_h->getRows($sort);
                }
            }else{
              $tplc_h= $this->m_tplc_h->getRows($sort);
            }
    ...
    function getRowsByInstance() {
        log_message('debug', 'TPLC_H.getRowsByInstance post : '.json_encode($this->input->post(NULL, FALSE)));
            $group = $this->input->get('group', FALSE); 
           $sort = $this->input->get('sort', FALSE);
           if(!$sort && $group) $sort = $group;
           if(!$sort || $group == $sort) 
        ...
        if (strlen($InstId) > 0) {
            $tplc_h= $this->m_tplc_h->getRowsByInstance($InstId,$sort);
            $return = array(
                'success' =>  TRUE ,
                'data'    => $tplc_h
            );
    ...
    function getRowsByParent() {
        log_message('debug', 'TPLC_H.getRowsByParent post : '.json_encode($this->input->post(NULL, FALSE)));
            $group = $this->input->get('group', FALSE); 
           $sort = $this->input->get('sort', FALSE);
           if(!$sort && $group) $sort = $group;
           if(!$sort || $group == $sort) 
        ...
        if (strlen($ParentId) > 0) {
            $tplc_h= $this->m_tplc_h->getRowsByParent($ParentId,$sort);
            $return = array(
                'success' => TRUE,
                'data'    => $tplc_h
            );
    ...
    function deleteRow() {
        log_message('debug', 'TPLC_H.deleteRow post : '.json_encode($this->input->post(NULL, FALSE)));
        $tempId  =  $this->input->get_post('tplc_hid', TRUE);
        if (strlen($tempId) > 0) {
            $return = $this->m_tplc_h->deleteRow($tempId);
        }
        else {
            $return = array(
                'success' => FALSE,
    ...
    private function _loadModels () {
        $this->load->model('M_tplc_h', 'm_tplc_h');
    }
}
?>