<?php
	 class C_tplc_m extends CI_Controller {
    function __construct() {
         parent::__construct();
        $this->_loadModels();
    }
    function setRow() {
          log_message('debug', 'TPLC_M.setRow post : '.json_encode($this->input->post(NULL, FALSE)));
          log_message('debug', 'TPLC_M.getRows get : '.json_encode($this->input->get(NULL, FALSE)));
          $data = array(
                'tplc_mid' =>  $this->input->get_post('tplc_mid', TRUE)
            ...
            $tplc_m = $this->m_tplc_m->setRow($data);
            print json_encode($tplc_m);
    }
    function newRow() {
            log_message('debug', 'TPLC_M.newRow post : '.json_encode($this->input->post(NULL, FALSE)));
          $data = array(
                'tplc_mid' =>  $this->input->get_post('tplc_mid', TRUE)
                ,'instanceid' =>  $this->input->get_post('instanceid', TRUE)
                ,'dcall' =>   $this->input->get_post('dcall', TRUE)
                ...
                $instanceid =  $this->input->get_post('instanceid', TRUE);
            $tplc_m= $this->m_tplc_m->newRow($instanceid,$data);
            $return = $tplc_m;
            print json_encode($return);
    }
    function getRow() {
        log_message('debug', 'TPLC_M.getRow post : '.json_encode($this->input->post(NULL, FALSE)));
        $empId  =  $this->input->get_post('id', TRUE);
        if (isset($empId)) {
            $tplc_m = $this->m_tplc_m->getRow($empId);
            $return = array(
                'success' => true,
                'data'    => $tplc_m
            );
    ...
    function getRows() {
           ...
           if(!$sort || $group == $sort) 
            ...
            if(isset($instanceid)){
                if($instanceid!=''){
                    $tplc_m= $this->m_tplc_m->getRowsByInstance($instanceid,$sort);
                }else{
                    $tplc_m= $this->m_tplc_m->getRows($sort);
                }
            }else{
              $tplc_m= $this->m_tplc_m->getRows($sort);
            }
    ...
    function getRowsByInstance() {
        log_message('debug', 'TPLC_M.getRowsByInstance post : '.json_encode($this->input->post(NULL, FALSE)));
            $group = $this->input->get('group', FALSE); 
           $sort = $this->input->get('sort', FALSE);
           if(!$sort && $group) $sort = $group;
           if(!$sort || $group == $sort) 
        ...
        if (strlen($InstId) > 0) {
            $tplc_m= $this->m_tplc_m->getRowsByInstance($InstId,$sort);
            $return = array(
                'success' =>  TRUE ,
                'data'    => $tplc_m
            );
    ...
    function getRowsByParent() {
        log_message('debug', 'TPLC_M.getRowsByParent post : '.json_encode($this->input->post(NULL, FALSE)));
            $group = $this->input->get('group', FALSE); 
           $sort = $this->input->get('sort', FALSE);
           if(!$sort && $group) $sort = $group;
           if(!$sort || $group == $sort) 
        ...
        if (strlen($ParentId) > 0) {
            $tplc_m= $this->m_tplc_m->getRowsByParent($ParentId,$sort);
            $return = array(
                'success' => TRUE,
                'data'    => $tplc_m
            );
    ...
    function deleteRow() {
        log_message('debug', 'TPLC_M.deleteRow post : '.json_encode($this->input->post(NULL, FALSE)));
        $tempId  =  $this->input->get_post('tplc_mid', TRUE);
        if (strlen($tempId) > 0) {
            $return = $this->m_tplc_m->deleteRow($tempId);
        }
        else {
            $return = array(
                'success' => FALSE,
    ...
    private function _loadModels () {
        $this->load->model('M_tplc_m', 'm_tplc_m');
    }
}
?>