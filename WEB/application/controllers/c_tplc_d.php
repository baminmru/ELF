<?php
	 class C_tplc_d extends CI_Controller {
    function __construct() {
         parent::__construct();
        $this->_loadModels();
    }
    function setRow() {
          log_message('debug', 'TPLC_D.setRow post : '.json_encode($this->input->post(NULL, FALSE)));
          log_message('debug', 'TPLC_D.getRows get : '.json_encode($this->input->get(NULL, FALSE)));
          $data = array(
                'tplc_did' =>  $this->input->get_post('tplc_did', TRUE)
            ...
            $tplc_d = $this->m_tplc_d->setRow($data);
            print json_encode($tplc_d);
    }
    function newRow() {
            log_message('debug', 'TPLC_D.newRow post : '.json_encode($this->input->post(NULL, FALSE)));
          $data = array(
                'tplc_did' =>  $this->input->get_post('tplc_did', TRUE)
                ,'instanceid' =>  $this->input->get_post('instanceid', TRUE)
                ,'dcall' =>   $this->input->get_post('dcall', TRUE)
                ...
                $instanceid =  $this->input->get_post('instanceid', TRUE);
            $tplc_d= $this->m_tplc_d->newRow($instanceid,$data);
            $return = $tplc_d;
            print json_encode($return);
    }
    function getRow() {
        log_message('debug', 'TPLC_D.getRow post : '.json_encode($this->input->post(NULL, FALSE)));
        $empId  =  $this->input->get_post('id', TRUE);
        if (isset($empId)) {
            $tplc_d = $this->m_tplc_d->getRow($empId);
            $return = array(
                'success' => true,
                'data'    => $tplc_d
            );
    ...
    function getRows() {
           ...
           if(!$sort || $group == $sort) 
            ...
            if(isset($instanceid)){
                if($instanceid!=''){
                    $tplc_d= $this->m_tplc_d->getRowsByInstance($instanceid,$sort);
                }else{
                    $tplc_d= $this->m_tplc_d->getRows($sort);
                }
            }else{
              $tplc_d= $this->m_tplc_d->getRows($sort);
            }
    ...
    function getRowsByInstance() {
        log_message('debug', 'TPLC_D.getRowsByInstance post : '.json_encode($this->input->post(NULL, FALSE)));
            $group = $this->input->get('group', FALSE); 
           $sort = $this->input->get('sort', FALSE);
           if(!$sort && $group) $sort = $group;
           if(!$sort || $group == $sort) 
        ...
        if (strlen($InstId) > 0) {
            $tplc_d= $this->m_tplc_d->getRowsByInstance($InstId,$sort);
            $return = array(
                'success' =>  TRUE ,
                'data'    => $tplc_d
            );
    ...
    function getRowsByParent() {
        log_message('debug', 'TPLC_D.getRowsByParent post : '.json_encode($this->input->post(NULL, FALSE)));
            $group = $this->input->get('group', FALSE); 
           $sort = $this->input->get('sort', FALSE);
           if(!$sort && $group) $sort = $group;
           if(!$sort || $group == $sort) 
        ...
        if (strlen($ParentId) > 0) {
            $tplc_d= $this->m_tplc_d->getRowsByParent($ParentId,$sort);
            $return = array(
                'success' => TRUE,
                'data'    => $tplc_d
            );
    ...
    function deleteRow() {
        log_message('debug', 'TPLC_D.deleteRow post : '.json_encode($this->input->post(NULL, FALSE)));
        $tempId  =  $this->input->get_post('tplc_did', TRUE);
        if (strlen($tempId) > 0) {
            $return = $this->m_tplc_d->deleteRow($tempId);
        }
        else {
            $return = array(
                'success' => FALSE,
    ...
    private function _loadModels () {
        $this->load->model('M_tplc_d', 'm_tplc_d');
    }
}
?>