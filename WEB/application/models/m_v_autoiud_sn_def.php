      ...
      function getRowsSL($offset,$limit, $sort = array(), $filter = null)
{
    ...
    try{
	 ...
	 if (isset($offset) && isset($limit)) {
	    $res = $this->jservice->get(array('Action' => 'GetViewData', 'ViewName' => 'v_autoiud_sn_def','FieldList'=>'instanceid,id,iud_sn_def_sequence,iud_sn_def_name','Sort'=>$sort, 'WhereClause' => $whereclause,'Limit'=>$limit,'Offset'=>$offset));
	} else {
	    $res = $this->jservice->get(array('Action' => 'GetViewData', 'ViewName' => 'v_autoiud_sn_def','FieldList'=>'instanceid,id,iud_sn_def_sequence,iud_sn_def_name','Sort'=>$sort, 'WhereClause' => $whereclause));
	}
	$root = new stdClass();
	$root->total = $this->jservice->get(array('Action' => 'CountView', 'ViewName' => 'v_autoiud_sn_def','FieldList'=>'instanceid,id,iud_sn_def_sequence,iud_sn_def_name', 'WhereClause' => $whereclause));
	$root->success = true;
	$root->rows = $res;
	return $root;
}
    function deleteRow($id = null) {
	    $this->jservice->get(array('Action'=>'DeleteDocument', 'TypeName'=>'iud_sn', 'DocumentID'=>$id));
