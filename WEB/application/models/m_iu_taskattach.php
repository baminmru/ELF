...
//////////////////////////// end support file data for: theref
    ...
    function getRowsByInstance($id,$sort=array())
		{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_taskattachid) as iu_taskattachid, B2G(iu_taskattachid) as id,B2G(instanceid) as instanceid, iu_taskattach_BRIEF_F(iu_taskattachid , NULL) as  brief,B2G(dtype) dtype, iud_doctype_BRIEF_F(dtype, NULL) as dtype_grid,B2G(filereftype) filereftype, iud_rt_def_BRIEF_F(filereftype, NULL) as filereftype_grid,theref,theref_ext,fileurl,filetext,info', 'ViewName' => 'iu_taskattach', 'WhereClause' => 'instanceid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
	...
	{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_taskattachid) as iu_taskattachid, B2G(iu_taskattachid) as id,B2G(instanceid) as instanceid, iu_taskattach_BRIEF_F(iu_taskattachid , NULL) as  brief,B2G(dtype) dtype, iud_doctype_BRIEF_F(dtype, NULL) as dtype_grid,B2G(filereftype) filereftype, iud_rt_def_BRIEF_F(filereftype, NULL) as filereftype_grid,theref,theref_ext,fileurl,filetext,info', 'ViewName' => 'iu_taskattach', 'WhereClause' => ' parentstructrowid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
