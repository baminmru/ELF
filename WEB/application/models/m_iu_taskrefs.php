...
//////////////////////////// end support file data for: theref
    ...
    function getRowsByInstance($id,$sort=array())
		{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_taskrefsid) as iu_taskrefsid, B2G(iu_taskrefsid) as id,B2G(instanceid) as instanceid, iu_taskrefs_BRIEF_F(iu_taskrefsid , NULL) as  brief,dtypename,B2G(filereftype) filereftype, iud_rt_def_BRIEF_F(filereftype, NULL) as filereftype_grid,theref,theref_ext,fileurl,filetext,info', 'ViewName' => 'iu_taskrefs', 'WhereClause' => 'instanceid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
	...
	{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_taskrefsid) as iu_taskrefsid, B2G(iu_taskrefsid) as id,B2G(instanceid) as instanceid, iu_taskrefs_BRIEF_F(iu_taskrefsid , NULL) as  brief,dtypename,B2G(filereftype) filereftype, iud_rt_def_BRIEF_F(filereftype, NULL) as filereftype_grid,theref,theref_ext,fileurl,filetext,info', 'ViewName' => 'iu_taskrefs', 'WhereClause' => ' parentstructrowid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
