...
class  M_iu_crole extends CI_Model {
	...
	if (!empty($empId)){
	    $res = $this->jservice->get(array('Action' => 'GetRowData','FieldList'=>'B2G(iu_croleid) as iu_croleid, B2G(iu_croleid) as id,B2G(instanceid) as instanceid, iu_crole_BRIEF_F(iu_croleid , NULL) as  brief,name,allowsetuser, case allowsetuser  when -1 then \'Да\' when 0 then \'Нет\' else \'\'  end   as allowsetuser_grid', 'PartName' => 'iu_crole', 'ID' =>  $empId 	));
	    if (!empty($res)) {
	        $result = $res[0];
	    }
	}
    ...
    function getRows($sort=array())
		{
	    $res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_croleid) as iu_croleid, B2G(iu_croleid) as id,B2G(instanceid) as instanceid, iu_crole_BRIEF_F(iu_croleid , NULL) as  brief,name,allowsetuser, case allowsetuser  when -1 then \'Да\' when 0 then \'Нет\' else \'\'  end   as allowsetuser_grid', 'ViewName' => 'iu_crole'));
	    if (count($res)) {
	        return $res;
	    } else {
	        return null;
