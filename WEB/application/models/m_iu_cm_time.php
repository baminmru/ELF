...
class  M_iu_cm_time extends CI_Model {
    ...
    function getRows($sort=array())
		{
	    $res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_cm_timeid) as iu_cm_timeid, B2G(iu_cm_timeid) as id,B2G(instanceid) as instanceid, iu_cm_time_BRIEF_F(iu_cm_timeid , NULL) as  brief,starttime,  DATE_FORMAT(thedate,\'%Y-%m-%d %H:%i:%s\') as  thedate,ischecked, case ischecked  when -1 then \'Да\' when 0 then \'Нет\'End  as ischecked_grid,info,B2G(TheAuthor) theauthor, iu_u_def_BRIEF_F(theauthor, NULL) as theauthor_grid,endtime,B2G(messagetype) messagetype, iud_mt_def_BRIEF_F(messagetype, NULL) as messagetype_grid', 'ViewName' => 'iu_cm_time'));
	    if (count($res)) {
	        return $res;
	    } else {
	        return null;
    ...
    function getRowsByInstance($id,$sort=array())
		{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_cm_timeid) as iu_cm_timeid, B2G(iu_cm_timeid) as id,B2G(instanceid) as instanceid, iu_cm_time_BRIEF_F(iu_cm_timeid , NULL) as  brief,starttime,  DATE_FORMAT(thedate,\'%Y-%m-%d %H:%i:%s\') as  thedate,ischecked, case ischecked  when -1 then \'Да\' when 0 then \'Нет\'End  as ischecked_grid,info,B2G(TheAuthor) theauthor, iu_u_def_BRIEF_F(theauthor, NULL) as theauthor_grid,endtime,B2G(messagetype) messagetype, iud_mt_def_BRIEF_F(messagetype, NULL) as messagetype_grid', 'ViewName' => 'iu_cm_time', 'WhereClause' => 'instanceid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
	...
	{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_cm_timeid) as iu_cm_timeid, B2G(iu_cm_timeid) as id,B2G(instanceid) as instanceid, iu_cm_time_BRIEF_F(iu_cm_timeid , NULL) as  brief,starttime,  DATE_FORMAT(thedate,\'%Y-%m-%d %H:%i:%s\') as  thedate,ischecked, case ischecked  when -1 then \'Да\' when 0 then \'Нет\'End  as ischecked_grid,info,B2G(TheAuthor) theauthor, iu_u_def_BRIEF_F(theauthor, NULL) as theauthor_grid,endtime,B2G(messagetype) messagetype, iud_mt_def_BRIEF_F(messagetype, NULL) as messagetype_grid', 'ViewName' => 'iu_cm_time', 'WhereClause' => ' parentstructrowid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
