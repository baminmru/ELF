...
class  M_iu_tm_acts extends CI_Model {
	...
	if (!empty($empId)){
	    $res = $this->jservice->get(array('Action' => 'GetRowData','FieldList'=>'B2G(iu_tm_actsid) as iu_tm_actsid, B2G(iu_tm_actsid) as id,B2G(parentstructrowid) as parentid, iu_tm_acts_BRIEF_F(iu_tm_actsid , NULL) as  brief,aktnum,aktfile,aktfile_ext,quantity,payment,  DATE_FORMAT(paymentdate,\'%Y-%m-%d\') as  paymentdate,msgfile,msgfile_ext,paymentfile,paymentfile_ext,avancefile,avancefile_ext,thecomment', 'PartName' => 'iu_tm_acts', 'ID' =>  $empId 	));
	    if (!empty($res)) {
	        $result = $res[0];
	    }
	}
...
//////////////////////////// end support file data for: avancefile
	if (!empty($data)) {
	    ...
	    return array('success' => TRUE, 'data' => $this->getRow($data['iu_tm_actsid'] ));
	} else {
    ...
    function getRows($sort=array())
		{
	    $res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_tm_actsid) as iu_tm_actsid, B2G(iu_tm_actsid) as id,B2G(parentstructrowid) as parentid, iu_tm_acts_BRIEF_F(iu_tm_actsid , NULL) as  brief,aktnum,aktfile,aktfile_ext,quantity,payment,  DATE_FORMAT(paymentdate,\'%Y-%m-%d\') as  paymentdate,msgfile,msgfile_ext,paymentfile,paymentfile_ext,avancefile,avancefile_ext,thecomment', 'ViewName' => 'iu_tm_acts'));
	    if (count($res)) {
	        return $res;
	    } else {
	        return null;
    ...
    function getRowsByInstance($id,$sort=array())
		{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_tm_actsid) as iu_tm_actsid, B2G(iu_tm_actsid) as id,B2G(parentstructrowid) as parentid, iu_tm_acts_BRIEF_F(iu_tm_actsid , NULL) as  brief,aktnum,aktfile,aktfile_ext,quantity,payment,  DATE_FORMAT(paymentdate,\'%Y-%m-%d\') as  paymentdate,msgfile,msgfile_ext,paymentfile,paymentfile_ext,avancefile,avancefile_ext,thecomment', 'ViewName' => 'iu_tm_acts', 'WhereClause' => 'instanceid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
	...
	{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_tm_actsid) as iu_tm_actsid, B2G(iu_tm_actsid) as id,B2G(parentstructrowid) as parentid, iu_tm_acts_BRIEF_F(iu_tm_actsid , NULL) as  brief,aktnum,aktfile,aktfile_ext,quantity,payment,  DATE_FORMAT(paymentdate,\'%Y-%m-%d\') as  paymentdate,msgfile,msgfile_ext,paymentfile,paymentfile_ext,avancefile,avancefile_ext,thecomment', 'ViewName' => 'iu_tm_acts', 'WhereClause' => ' parentstructrowid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
