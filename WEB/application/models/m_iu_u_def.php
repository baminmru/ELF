...
class  M_iu_u_def extends CI_Model {
	...
	if (!empty($empId)){
	    $res = $this->jservice->get(array('Action' => 'GetRowDataTemp','FieldList'=>'B2G(iu_u_defid) as iu_u_defid, B2G(iu_u_defid) as id,B2G(instanceid) as instanceid, iu_u_def_BRIEF_F(iu_u_defid , NULL) as  brief,thephone,surname,email,sendtomail, case sendtomail  when -1 then \'Да\' when 0 then \'Нет\' else \'\'  end   as sendtomail_grid,B2G(theClient) theclient, bpc_info_BRIEF_F(theclient, NULL) as theclient_grid,login,name,lastname,B2G(curRole) currole, iu_crole_BRIEF_F(currole, NULL) as currole_grid,freelancer, case freelancer  when -1 then \'Да\' when 0 then \'Нет\' else \'\'  end   as freelancer_grid', 'PartName' => 'iu_u_def', 'ID' =>  $empId 	));
	    if (!empty($res)) {
	        $result = $res[0];
	    }
	}
    ...
    function deleteRow($id = null) {
	  if (!empty($id) && $this->jservice->get(array('Action' => 'DeleteRow', 'PartName' => 'iu_u_def', 'RowID' => $id)) == 'OK')
