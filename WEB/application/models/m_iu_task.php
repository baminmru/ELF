...
class  M_iu_task extends CI_Model {
	...
	if (!empty($empId)){
	    $res = $this->jservice->get(array('Action' => 'GetRowData','FieldList'=>'B2G(iu_taskid) as iu_taskid, B2G(iu_taskid) as id,B2G(instanceid) as instanceid, iu_task_BRIEF_F(iu_taskid , NULL) as  brief,B2G(doer) doer, iu_u_def_BRIEF_F(doer, NULL) as doer_grid,subj,  DATE_FORMAT(createdate,\'%Y-%m-%d %H:%i:%s\') as  createdate,info,  DATE_FORMAT(planenddate,\'%Y-%m-%d %H:%i:%s\') as  planenddate,B2G(contoller) contoller, iu_u_def_BRIEF_F(contoller, NULL) as contoller_grid,doer_comment,B2G(doer_states) doer_states, iud_sn_def_BRIEF_F(doer_states, NULL) as doer_states_grid,controller_comment,taskfinished, case taskfinished  when -1 then \'Да\' when 0 then \'Нет\'End  as taskfinished_grid,ischecked, case ischecked  when -1 then \'Да\' when 0 then \'Нет\'End  as ischecked_grid,  DATE_FORMAT(finishdate,\'%Y-%m-%d %H:%i:%s\') as  finishdate,taskcancelled, case taskcancelled  when -1 then \'Да\' when 0 then \'Нет\'End  as taskcancelled_grid,  DATE_FORMAT(senttodoer,\'%Y-%m-%d %H:%i:%s\') as  senttodoer,B2G(TheProcess) theprocess, iu_urok_def_BRIEF_F(theprocess, NULL) as theprocess_grid,B2G(ProcessStatus) processstatus, iu_status_BRIEF_F(processstatus, NULL) as processstatus_grid,B2G(StateTask) statetask, iu_statustask_BRIEF_F(statetask, NULL) as statetask_grid,B2G(delegatefrom) delegatefrom, iu_task_BRIEF_F(delegatefrom, NULL) as delegatefrom_grid', 'PartName' => 'iu_task', 'ID' =>  $empId 	));
	    if (!empty($res)) {
	        $result = $res[0];
	    }
	}
