...
listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Управление видимостью'});}}
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('bpdi.edit')!=0 && OTAllowDelete('bpdi')){
        Ext.Msg.show({
            title:  'Удалить?',
    ...
    onAddClick: function(){
   	    if(CheckOperation('bpdi.edit')!=0 && OTAllowAdd('bpdi')){
            Ext.Ajax.request({
                ...
                var p=eval('bpdi_Panel_'+OTAddMode('bpdi')+'( res.data, false,null )') ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        var selection = this.getView().getSelectionModel().getSelection()[0];
        if (selection) {
   	    if(CheckOperation('bpdi.edit')!=0 ){
                var edit = Ext.create('iu.windowObjects');
                edit.prefix='c_v_autoiu_int_modules';
                edit.setTitle('Редактирование документа: Интерфейс') ;
            var p=eval('bpdi_Panel_'+OTEditMode('bpdi')+'( selection.get(\'instanceid\'), false, selection )') ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function bpdi_Jrnl(){ 
  ...
  var bpdi= Ext.create('Ext.form.Panel', {
       closable: true,
       id:     'bpdi_jrnl',
       title: 'Интерфейс',
      layout: 'fit',
