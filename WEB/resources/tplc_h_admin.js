...
function DefineInterface_tplc_h_admin(id,mystore){

var groupingFeature_tplc_h = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var filterFeature_tplc_h = {
...
local: true 
 ...
 var p1;
    function onDeleteConfirm(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_tplc_h/deleteRow',
            method:  'POST',
    		params: { 
    				tplc_hid: selection.get('tplc_hid')
    				}
    	});
    ...
    function onAddClick(){
   	if(CheckOperation('TPLC.edit')!=0){
                var edit = Ext.create('EditWindow_tplc_hadmin');
                p1.store.insert(0, new model_tplc_h());
 ...
 p1=   new Ext.grid.Panel(
      ...
      stateful:stateFulSystem,
       stateId:  'tplc_hadmin',
        ...
        frame: true,
        instanceid: '',
        features: [groupingFeature_tplc_h,filterFeature_tplc_h],
          dockedItems: [{
                xtype:  'toolbar',
                items: [
                {
...
function DefineForms_tplc_h_admin(){


Ext.define('Form_tplc_hadmin', {
extend:  'Ext.form.Panel',
alias: 'widget.f_tplc_hadmin',
...
allowBlank:true
    ...
    onSave: function(){
        ...
        if (form.isValid()) {
            ...
            Ext.Ajax.request({
                url: rootURL+'index.php/c_tplc_h/setRow',
                method:  'POST',
                params: { 
                    instanceid: this.instanceid
                    ,tplc_hid: active.get('tplc_hid')
	            ...
	            if(res.success==false){
                    ...
                    if(active.get('tplc_hid')==''){
               			active.set('tplc_hid',res.data['tplc_hid']);
                    }
    ...
    onReset: function(){
        if(this.activeRecord.get('tplc_hid')==''){
...
Ext.define('EditWindow_tplc_hadmin', {
