...
var groupingFeature_autobpc_info = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var interval_autobpc_info;
Ext.define('grid_autobpc_info', {
    extend:  'Ext.grid.Panel',
    alias: 'widget.g_v_autobpc_info',
    requires: [
        'Ext.grid.*',
        'Ext.form.field.Text',
    ...
    initComponent: function(){
        Ext.apply(this, {
        frame: false,
        store: store_v_autobpc_info,
        features: [groupingFeature_autobpc_info],
        ...
        columns: [
            {text: "Название", width:800, dataIndex: 'bpc_info_name', sortable: true}
        ]
        ,
...
listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Название'});}}
    ...
    listeners: {
        ...
        ,
        	added:function(){
        			//interval_autobpc_info= setInterval(function() {  
        			//	store_v_autobpc_info.load();
        			//}, 60000);
        		}
        	,
        	destroy:function(){
        		//clearInterval(interval_autobpc_info);
        }
    },
    onDeleteConfirm:function(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_v_autobpc_info/deleteRow',
            method:  'POST',
    		params: { 
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('bpc.edit')!=0 && OTAllowDelete('bpc')){
        Ext.Msg.show({
            title:  'Удалить?',
    ...
    onAddClick: function(){
   	    if(CheckOperation('bpc.edit')!=0 && OTAllowAdd('bpc')){
            Ext.Ajax.request({
                url: rootURL+'index.php/c_v_autobpc_info/newRow',
                method:  'POST',
                params: { 
                },
                ...
                edit.prefix='c_v_autobpc_info';
                edit.setTitle('Создание документа:Клиент') ;
                var p=eval('bpc_Panel_'+OTAddMode('bpc')+'( res.data, false,null )') ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        var selection = this.getView().getSelectionModel().getSelection()[0];
        if (selection) {
   	    if(CheckOperation('bpc.edit')!=0 ){
                var edit = Ext.create('iu.windowObjects');
                edit.prefix='c_v_autobpc_info';
                edit.setTitle('Редактирование документа: Клиент') ;
            var p=eval('bpc_Panel_'+OTEditMode('bpc')+'( selection.get(\'instanceid\'), false, selection )') ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function bpc_Jrnl(){ 
  ...
  var bpc= Ext.create('Ext.form.Panel', {
       closable: true,
       id:     'bpc_jrnl',
       title: 'Клиент',
      layout: 'fit',
      ...
      fieldDefaults: {
        ...
        items: [{
            itemId:'gr_autobpc_info',
            xtype:'g_v_autobpc_info',
            stateful: stateFulSystem,
            stateId:'j_v_autobpc_info',
            layout: 'fit',
            flex: 1,
            store: store_v_autobpc_info
    }] // tabpanel
