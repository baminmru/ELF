...
var groupingFeature_autotplc_header = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var interval_autotplc_header;
Ext.define('grid_autotplc_header', {
    extend:  'Ext.grid.Panel',
    alias: 'widget.g_v_autotplc_header',
    requires: [
        'Ext.grid.*',
        'Ext.form.field.Text',
    ...
    initComponent: function(){
        Ext.apply(this, {
        frame: false,
        store: store_v_autotplc_header,
        features: [groupingFeature_autotplc_header],
        ...
        columns: [
            {text: "Устройство", width:800, dataIndex: 'tplc_header_id_bd', sortable: true}
        ]
        ,
...
displayField:   'brief',
typeAhead: true,
name:   'tplc_header_id_bd_id',
itemId:   'tplc_header_id_bd_id',
fieldLabel:  '',
...
hideLabel:  true
    ...
    listeners: {
        ...
        ,
        	added:function(){
        			//interval_autotplc_header= setInterval(function() {  
        			//	store_v_autotplc_header.load();
        			//}, 60000);
        		}
        	,
        	destroy:function(){
        		//clearInterval(interval_autotplc_header);
        }
    },
    onDeleteConfirm:function(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_v_autotplc_header/deleteRow',
            method:  'POST',
    		params: { 
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('TPLC.edit')!=0 && OTAllowDelete('TPLC')){
        Ext.Msg.show({
            title:  'Удалить?',
    ...
    onAddClick: function(){
   	    if(CheckOperation('TPLC.edit')!=0 && OTAllowAdd('TPLC')){
            Ext.Ajax.request({
                url: rootURL+'index.php/c_v_autotplc_header/newRow',
                method:  'POST',
                params: { 
                },
                ...
                edit.prefix='c_v_autotplc_header';
                edit.setTitle('Создание документа:Данные') ;
                var p=eval('TPLC_Panel_'+OTAddMode('TPLC')+'( res.data, false,null )') ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        ...
        if (selection) {
   	    if(CheckOperation('TPLC.edit')!=0 ){
                var edit = Ext.create('iu.windowObjects');
                edit.prefix='c_v_autotplc_header';
                edit.setTitle('Редактирование документа: Данные') ;
            var p=eval('TPLC_Panel_'+OTEditMode('TPLC')+'( selection.get(\'instanceid\'), false, selection )') ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function TPLC_Jrnl(){ 
  ...
  var TPLC= Ext.create('Ext.form.Panel', {
       closable: true,
       id:     'tplc_jrnl',
       title: 'Данные',
      layout: 'fit',
      ...
      fieldDefaults: {
        ...
        items: [{
            itemId:'gr_autotplc_header',
            xtype:'g_v_autotplc_header',
            stateful: stateFulSystem,
            stateId:'j_v_autotplc_header',
            layout: 'fit',
            flex: 1,
            store: store_v_autotplc_header
    }] // tabpanel
