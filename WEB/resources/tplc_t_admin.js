...
function DefineInterface_tplc_t_admin(id,mystore){

var groupingFeature_tplc_t = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var filterFeature_tplc_t = {
...
local: true 
 ...
 var p1;
    function onDeleteConfirm(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_tplc_t/deleteRow',
            method:  'POST',
    		params: { 
    				tplc_tid: selection.get('tplc_tid')
    				}
    	});
    ...
    function onAddClick(){
   	if(CheckOperation('TPLC.edit')!=0){
                var edit = Ext.create('EditWindow_tplc_tadmin');
                p1.store.insert(0, new model_tplc_t());
 ...
 p1=   new Ext.grid.Panel(
      ...
      stateful:stateFulSystem,
       stateId:  'tplc_tadmin',
        ...
        frame: true,
        instanceid: '',
        features: [groupingFeature_tplc_t,filterFeature_tplc_t],
          dockedItems: [{
                xtype:  'toolbar',
                items: [
                {
...
function DefineForms_tplc_t_admin(){


Ext.define('Form_tplc_tadmin', {
extend:  'Ext.form.Panel',
alias: 'widget.f_tplc_tadmin',
...
allowBlank:true
    ...
    onSave: function(){
        ...
        if (form.isValid()) {
            ...
            Ext.Ajax.request({
                url: rootURL+'index.php/c_tplc_t/setRow',
                method:  'POST',
                params: { 
                    instanceid: this.instanceid
                    ,tplc_tid: active.get('tplc_tid')
	            ...
	            if(res.success==false){
                    ...
                    if(active.get('tplc_tid')==''){
               			active.set('tplc_tid',res.data['tplc_tid']);
                    }
    ...
    onReset: function(){
        if(this.activeRecord.get('tplc_tid')==''){
...
Ext.define('EditWindow_tplc_tadmin', {
