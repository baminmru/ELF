...
function DefineInterface_tplt_plancall_(id,mystore){
...
name:   'cel_grid',
itemId:   'cel_grid',
listeners:{  select: function ( combo, records, eOpts ) {combo.up('form' ).activeRecord.set('cel', records[0].get('value'));}  },
fieldLabel:  'Опрашивать Эл.',
allowBlank:true
}
,
{
...
name:   'dnextel',
itemId:   'dnextel',
fieldLabel:  'Дата следующего опроса',
allowBlank:true
}
       ], 
       height: 70 
...
function DefineForms_tplt_plancall_(){


Ext.define('Form_tplt_plancall', {
extend:  'Ext.form.Panel',
alias: 'widget.f_tplt_plancall',
        ...
        if (form.isValid()) {
            ...
            Ext.Ajax.request({
                ...
                params: { 
                    ...
                    ,cel: active.get('cel') 
                    ,iel: active.get('iel') 
                    ,dnextel:function() { if(active.get('dnextel')) return active.get('dnextel').toLocaleFormat('%Y-%m-%d %H:%M:%S'); else return null;}()
                }
                , success: function(response){
                var text = response.responseText;
                var res =Ext.decode(text);
...
Ext.define('EditWindow_tplt_plancall', {
