...
Ext.define('grid_autoiu_rcfg_def', {
    ...
    initComponent: function(){
        ...
        columns: [
            {text: "Роль", width:800, dataIndex: 'iu_rcfg_def_therole', sortable: true}
        ]
        ,
...
enableRegEx: true,
queryMode:'local',
listeners:{  focus: function()   { if(this.store.count(false)==0) this.store.load();  } ,render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Роль'});} },
xtype:  'combobox',
store: cmbstore_iu_crole,
valueField:     'id',
displayField:   'brief',
typeAhead: true,
name:   'iu_rcfg_def_therole_id',
itemId:   'iu_rcfg_def_therole_id',
fieldLabel:  '',
emptyText:      'Роль',
hideLabel:  true
}
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('bprcfg.edit')!=0 && OTAllowDelete('bprcfg')){
        Ext.Msg.show({
            title:  'Удалить?',
    ...
    onAddClick: function(){
   	    if(CheckOperation('bprcfg.edit')!=0 && OTAllowAdd('bprcfg')){
            Ext.Ajax.request({
                ...
                var p=eval('bprcfg_Panel_'+OTAddMode('bprcfg')+'( res.data, false,null )') ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        ...
        if (selection) {
   	    if(CheckOperation('bprcfg.edit')!=0 ){
            ...
            var p=eval('bprcfg_Panel_'+OTEditMode('bprcfg')+'( selection.get(\'instanceid\'), false, selection )') ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function bprcfg_Jrnl(){ 

  var bprcfg= Ext.create('Ext.form.Panel', {
       closable: true,
       id:     'bprcfg_jrnl',
...
Ext.define('ObjectWindow_bprcfg', {
