...
Ext.define('grid_autoiu_l_def', {
    ...
    initComponent: function(){
        ...
        columns: [
            {text: "Отправитель", width:160, dataIndex: 'iu_l_def_sender', sortable: true}
            ,
            {text: "Кому", width:160, dataIndex: 'iu_l_def_doer', sortable: true}
            ,
{text: "Описание", width: 200, dataIndex: 'iu_l_def_info', sortable: true,
 renderer: function(value){var S =new String(value);  S=S.replace(new RegExp('/>','g'),'');  S=S.replace(new RegExp('<','g'),''); S=S.replace(new RegExp('>','g'),''); if(S.length >255) S=S.substr(0,255); return S;}}
            ,
            {text: "Дата создания", width:160, dataIndex: 'iu_l_def_createdate', sortable: true}
            ,
            {text: "Прочитана", width:160, dataIndex: 'iu_l_def_readdate', sortable: true}
        ]
        ,
...
enableRegEx: true,
queryMode:'local',
listeners:{  focus: function()   { if(this.store.count(false)==0) this.store.load();  } ,render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Кому'});} },
xtype:  'combobox',
store: cmbstore_iu_u_def,
valueField:     'id',
displayField:   'brief',
...
listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Прочитана по'});}}
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('iu_l.edit')!=0 && OTAllowDelete('iu_l')){
        Ext.Msg.show({
            title:  'Удалить?',
