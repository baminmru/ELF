...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPQ_result.GetDataTable()
        dt.TableName = "TPQ_result"

        ts = New DataGridTableStyle
        ts.MappingName = "TPQ_result"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Текстовый результат"
        cs.MappingName = "TextResult"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Запись мгновенного архива"
        cs.MappingName = "MomentArch"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Запись часового архива"
        cs.MappingName = "HourArch"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Запись суточного архива"
        cs.MappingName = "DayArch"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Запись итогового архива"
        cs.MappingName = "TotalArch"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Обработан с ошибкой"
        cs.MappingName = "IsError"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Протокол"
        cs.MappingName = "LogMessage"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Время начала обработки"
        cs.MappingName = "StartTime"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Время завершения обработки"
        cs.MappingName = "EndTime"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPQ.TPQ.TPQ_result
        ed = item.FindRowObject("TPQ_result", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPQ_result.Add
          Else
              ed = Item.TPQ_result.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
