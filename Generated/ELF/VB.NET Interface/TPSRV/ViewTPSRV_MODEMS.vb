...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPSRV_MODEMS.GetDataTable()
        dt.TableName = "TPSRV_MODEMS"

        ts = New DataGridTableStyle
        ts.MappingName = "TPSRV_MODEMS"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Номер ком порта"
        cs.MappingName = "PortNum"
        cs.NullText = ""
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPSRV.TPSRV.TPSRV_MODEMS
        ed = item.FindRowObject("TPSRV_MODEMS", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPSRV_MODEMS.Add
          Else
              ed = Item.TPSRV_MODEMS.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
