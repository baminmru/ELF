...
''' </remarks>
    Public Overrides Function ShowPartEditForm(ByVal Mode As String, ByRef RowItem As LATIR2.Document.DocRow_Base, optional byval FormReadOnly as boolean = false) As Boolean
        ...
        If Mode = "" Then
            ...
            If RowItem.PartName.ToUpper = "TPSRV_INFO" Then
                Dim f As frmTPSRV_INFO
                f = New frmTPSRV_INFO
            ...
            If RowItem.PartName.ToUpper = "TPSRV_MODEMS" Then
                Dim f As frmTPSRV_MODEMS
                f = New frmTPSRV_MODEMS
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If

            If RowItem.PartName.ToUpper = "TPSRV_PORTS" Then
                Dim f As frmTPSRV_PORTS
                f = New frmTPSRV_PORTS
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If
