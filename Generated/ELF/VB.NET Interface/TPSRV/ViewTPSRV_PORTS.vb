...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPSRV_PORTS.GetDataTable()
        dt.TableName = "TPSRV_PORTS"

        ts = New DataGridTableStyle
        ts.MappingName = "TPSRV_PORTS"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Номер порта"
        cs.MappingName = "PortName"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Может использоваться сервером"
        cs.MappingName = "IsUsable"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Занят"
        cs.MappingName = "IsUsed"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Занят до"
        cs.MappingName = "UsedUntil"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPSRV.TPSRV.TPSRV_PORTS
        ed = item.FindRowObject("TPSRV_PORTS", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPSRV_PORTS.Add
          Else
              ed = Item.TPSRV_PORTS.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
