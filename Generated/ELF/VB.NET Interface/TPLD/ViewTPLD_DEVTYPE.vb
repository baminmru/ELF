...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPLD_DEVTYPE.GetDataTable()
        dt.TableName = "TPLD_DEVTYPE"

        ts = New DataGridTableStyle
        ts.MappingName = "TPLD_DEVTYPE"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Класс устройства"
        cs.MappingName = "DevClass"
        cs.NullText = ""
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Библиотека драйвера"
        cs.MappingName = "DriverLibName"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPLD.TPLD.TPLD_DEVTYPE
        ed = item.FindRowObject("TPLD_DEVTYPE", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPLD_DEVTYPE.Add
          Else
              ed = Item.TPLD_DEVTYPE.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
