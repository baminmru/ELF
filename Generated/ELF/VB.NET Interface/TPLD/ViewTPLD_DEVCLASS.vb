...
Public Class viewTPLD_DEVCLASS
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPLD_DEVCLASS.GetDataTable()
        dt.TableName = "TPLD_DEVCLASS"

        ts = New DataGridTableStyle
        ts.MappingName = "TPLD_DEVCLASS"
        ts.ReadOnly = True
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPLD.TPLD.TPLD_DEVCLASS
        ed = item.FindRowObject("TPLD_DEVCLASS", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPLD_DEVCLASS.Add
          Else
              ed = Item.TPLD_DEVCLASS.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
