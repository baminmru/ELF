...
''' </remarks>
    Public Overrides Function ShowPartEditForm(ByVal Mode As String, ByRef RowItem As LATIR2.Document.DocRow_Base, optional byval FormReadOnly as boolean = false) As Boolean
        ...
        If Mode = "" Then
            ...
            If RowItem.PartName.ToUpper = "TPLD_SNABTOP" Then
                Dim f As frmTPLD_SNABTOP
                f = New frmTPLD_SNABTOP
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If

            If RowItem.PartName.ToUpper = "TPLD_PARAM" Then
                Dim f As frmTPLD_PARAM
                f = New frmTPLD_PARAM
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            ...
            If RowItem.PartName.ToUpper = "TPLD_SNAB" Then
                Dim f As frmTPLD_SNAB
                f = New frmTPLD_SNAB
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            ...
            If RowItem.PartName.ToUpper = "TPLD_F" Then
                Dim f As frmTPLD_F
                f = New frmTPLD_F
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
            ...
            If RowItem.PartName.ToUpper = "TPLD_CONNECTTYPE" Then
                Dim f As frmTPLD_CONNECTTYPE
                f = New frmTPLD_CONNECTTYPE
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If

            If RowItem.PartName.ToUpper = "TPLD_DEVCLASS" Then
                Dim f As frmTPLD_DEVCLASS
                f = New frmTPLD_DEVCLASS
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If

            If RowItem.PartName.ToUpper = "TPLD_DEVTYPE" Then
                Dim f As frmTPLD_DEVTYPE
                f = New frmTPLD_DEVTYPE
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If

            If RowItem.PartName.ToUpper = "TPLD_GRP" Then
                Dim f As frmTPLD_GRP
                f = New frmTPLD_GRP
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            ...
            If RowItem.PartName.ToUpper = "TPLD_PARAMTYPE" Then
                Dim f As frmTPLD_PARAMTYPE
                f = New frmTPLD_PARAMTYPE
                f.Attach(RowItem, Me.GUIManager,FormReadOnly)
                ShowPartEditForm = (f.ShowDialog() = System.Windows.Forms.DialogResult.OK)
                f = Nothing
            End If
