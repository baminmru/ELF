...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPLD_GRP.GetDataTable()
        dt.TableName = "TPLD_GRP"

        ts = New DataGridTableStyle
        ts.MappingName = "TPLD_GRP"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Клиент"
        cs.MappingName = "theClient"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Название группы"
        cs.MappingName = "CGRPNM"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPLD.TPLD.TPLD_GRP
        ed = item.FindRowObject("TPLD_GRP", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPLD_GRP.Add
          Else
              ed = Item.TPLD_GRP.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
