...
Public Class viewTPLC_MISSINGadmin
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.gv.TabIndex = 0
        '
        'viewTPLC_MISSINGadmin
        '
        Me.Controls.Add (Me.gv)
        Me.name = "viewTPLC_MISSINGadmin"
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.TPLC_MISSING.GetDataTable()
        dt.TableName = "TPLC_MISSING"

        ts = New DataGridTableStyle
        ts.MappingName = "TPLC_MISSING"
        ts.ReadOnly = True
        ...
        cs.MappingName = "AType"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Дата архива"
        cs.MappingName = "ADate"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Количество попыток  опроса"
        cs.MappingName = "QueryCount"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As TPLC.TPLC.TPLC_MISSING
        ed = item.FindRowObject("TPLC_MISSING", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.TPLC_MISSING.Add
          Else
              ed = Item.TPLC_MISSING.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
