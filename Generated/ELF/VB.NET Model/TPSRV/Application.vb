...
''' <summary>
''' Внутренняя переменная для раздела Модемы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPSRV_MODEMS As TPSRV_MODEMS_col


''' <summary>
'''Свойство для доступа к строкам раздела Модемы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPSRV_MODEMS() As TPSRV_MODEMS_col
            Get
                If m_TPSRV_MODEMS Is Nothing Then
                    m_TPSRV_MODEMS = New TPSRV_MODEMS_col
                    m_TPSRV_MODEMS.Application = Me
                    m_TPSRV_MODEMS.Parent = Me
                    m_TPSRV_MODEMS.Refresh()
                End If
                TPSRV_MODEMS = m_TPSRV_MODEMS
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Описание сервера
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPSRV_INFO As TPSRV_INFO_col


''' <summary>
'''Свойство для доступа к строкам раздела Описание сервера
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPSRV_INFO() As TPSRV_INFO_col
            Get
                If m_TPSRV_INFO Is Nothing Then
                    m_TPSRV_INFO = New TPSRV_INFO_col
                    m_TPSRV_INFO.Application = Me
                    m_TPSRV_INFO.Parent = Me
                    m_TPSRV_INFO.Refresh()
                End If
                TPSRV_INFO = m_TPSRV_INFO
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Ком порты
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPSRV_PORTS As TPSRV_PORTS_col


''' <summary>
'''Свойство для доступа к строкам раздела Ком порты
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPSRV_PORTS() As TPSRV_PORTS_col
            Get
                If m_TPSRV_PORTS Is Nothing Then
                    m_TPSRV_PORTS = New TPSRV_PORTS_col
                    m_TPSRV_PORTS.Application = Me
                    m_TPSRV_PORTS.Parent = Me
                    m_TPSRV_PORTS.Refresh()
                End If
                TPSRV_PORTS = m_TPSRV_PORTS
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = TPSRV_INFO.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPSRV_MODEMS.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPSRV_PORTS.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("TPSRV_INFO_COL")
            TPSRV_INFO.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPSRV_MODEMS_COL")
            TPSRV_MODEMS.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPSRV_PORTS_COL")
            TPSRV_PORTS.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            TPSRV_INFO.XMLSave(node, Xdom)
            TPSRV_MODEMS.XMLSave(node, Xdom)
            TPSRV_PORTS.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    TPSRV_INFO.BatchUpdate
    TPSRV_MODEMS.BatchUpdate
    TPSRV_PORTS.BatchUpdate
End Sub
    End Class
End Namespace
