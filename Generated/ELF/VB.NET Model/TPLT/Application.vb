...
''' <summary>
'''Реализация документа Опрашиваемое устройство
''' </summary>
''' <remarks>
'''Устройство
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Отчеты
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLT_REPORTS As TPLT_REPORTS_col


''' <summary>
'''Свойство для доступа к строкам раздела Отчеты
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLT_REPORTS() As TPLT_REPORTS_col
            Get
                If m_TPLT_REPORTS Is Nothing Then
                    m_TPLT_REPORTS = New TPLT_REPORTS_col
                    m_TPLT_REPORTS.Application = Me
                    m_TPLT_REPORTS.Parent = Me
                    m_TPLT_REPORTS.Refresh()
                End If
                TPLT_REPORTS = m_TPLT_REPORTS
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Параметры соединения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLT_CONNECT As TPLT_CONNECT_col


''' <summary>
'''Свойство для доступа к строкам раздела Параметры соединения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLT_CONNECT() As TPLT_CONNECT_col
            Get
                If m_TPLT_CONNECT Is Nothing Then
                    m_TPLT_CONNECT = New TPLT_CONNECT_col
                    m_TPLT_CONNECT.Application = Me
                    m_TPLT_CONNECT.Parent = Me
                    m_TPLT_CONNECT.Refresh()
                End If
                TPLT_CONNECT = m_TPLT_CONNECT
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Граничные значения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLT_VALUEBOUNDS As TPLT_VALUEBOUNDS_col


''' <summary>
'''Свойство для доступа к строкам раздела Граничные значения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLT_VALUEBOUNDS() As TPLT_VALUEBOUNDS_col
            Get
                If m_TPLT_VALUEBOUNDS Is Nothing Then
                    m_TPLT_VALUEBOUNDS = New TPLT_VALUEBOUNDS_col
                    m_TPLT_VALUEBOUNDS.Application = Me
                    m_TPLT_VALUEBOUNDS.Parent = Me
                    m_TPLT_VALUEBOUNDS.Refresh()
                End If
                TPLT_VALUEBOUNDS = m_TPLT_VALUEBOUNDS
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела План опроса устройств
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLT_PLANCALL As TPLT_PLANCALL_col


''' <summary>
'''Свойство для доступа к строкам раздела План опроса устройств
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLT_PLANCALL() As TPLT_PLANCALL_col
            Get
                If m_TPLT_PLANCALL Is Nothing Then
                    m_TPLT_PLANCALL = New TPLT_PLANCALL_col
                    m_TPLT_PLANCALL.Application = Me
                    m_TPLT_PLANCALL.Parent = Me
                    m_TPLT_PLANCALL.Refresh()
                End If
                TPLT_PLANCALL = m_TPLT_PLANCALL
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Параметры для вывода
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLT_MASK As TPLT_MASK_col


''' <summary>
'''Свойство для доступа к строкам раздела Параметры для вывода
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLT_MASK() As TPLT_MASK_col
            Get
                If m_TPLT_MASK Is Nothing Then
                    m_TPLT_MASK = New TPLT_MASK_col
                    m_TPLT_MASK.Application = Me
                    m_TPLT_MASK.Parent = Me
                    m_TPLT_MASK.Refresh()
                End If
                TPLT_MASK = m_TPLT_MASK
            End Get
        End Property
...
'''
''' </remarks>
  Private m_TPLT_BDEVICES As TPLT_BDEVICES_col
...
'''
''' </remarks>
        Public ReadOnly Property TPLT_BDEVICES() As TPLT_BDEVICES_col
            Get
                If m_TPLT_BDEVICES Is Nothing Then
                    m_TPLT_BDEVICES = New TPLT_BDEVICES_col
                    m_TPLT_BDEVICES.Application = Me
                    m_TPLT_BDEVICES.Parent = Me
                    m_TPLT_BDEVICES.Refresh()
                End If
                TPLT_BDEVICES = m_TPLT_BDEVICES
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Договорные установки
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLT_CONTRACT As TPLT_CONTRACT_col


''' <summary>
'''Свойство для доступа к строкам раздела Договорные установки
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLT_CONTRACT() As TPLT_CONTRACT_col
            Get
                If m_TPLT_CONTRACT Is Nothing Then
                    m_TPLT_CONTRACT = New TPLT_CONTRACT_col
                    m_TPLT_CONTRACT.Application = Me
                    m_TPLT_CONTRACT.Parent = Me
                    m_TPLT_CONTRACT.Refresh()
                End If
                TPLT_CONTRACT = m_TPLT_CONTRACT
            End Get
        End Property
...
''' </remarks>
        ...
        Public Overrides Sub Dispose()
            TPLT_BDEVICES.Dispose()
            TPLT_CONNECT.Dispose()
            TPLT_CONTRACT.Dispose()
            TPLT_PLANCALL.Dispose()
            TPLT_VALUEBOUNDS.Dispose()
            TPLT_REPORTS.Dispose()
            TPLT_MASK.Dispose()
        End Sub
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = TPLT_BDEVICES.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLT_CONNECT.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLT_CONTRACT.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLT_PLANCALL.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLT_VALUEBOUNDS.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLT_REPORTS.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLT_MASK.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("TPLT_BDEVICES_COL")
            TPLT_BDEVICES.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLT_CONNECT_COL")
            TPLT_CONNECT.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLT_CONTRACT_COL")
            TPLT_CONTRACT.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLT_PLANCALL_COL")
            TPLT_PLANCALL.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLT_VALUEBOUNDS_COL")
            TPLT_VALUEBOUNDS.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLT_REPORTS_COL")
            TPLT_REPORTS.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLT_MASK_COL")
            TPLT_MASK.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            TPLT_BDEVICES.XMLSave(node, Xdom)
            TPLT_CONNECT.XMLSave(node, Xdom)
            TPLT_CONTRACT.XMLSave(node, Xdom)
            TPLT_PLANCALL.XMLSave(node, Xdom)
            TPLT_VALUEBOUNDS.XMLSave(node, Xdom)
            TPLT_REPORTS.XMLSave(node, Xdom)
            TPLT_MASK.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    TPLT_BDEVICES.BatchUpdate
    TPLT_CONNECT.BatchUpdate
    TPLT_CONTRACT.BatchUpdate
    TPLT_PLANCALL.BatchUpdate
    TPLT_VALUEBOUNDS.BatchUpdate
    TPLT_REPORTS.BatchUpdate
    TPLT_MASK.BatchUpdate
End Sub
    End Class
End Namespace
