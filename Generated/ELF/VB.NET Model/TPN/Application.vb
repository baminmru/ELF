...
''' <summary>
'''Реализация документа Здание,  место установки устройств
''' </summary>
''' <remarks>
'''Узел
''' </remarks>
...
'''
''' </remarks>
  Private m_TPN_DEF As TPN_DEF_col
...
'''
''' </remarks>
        Public ReadOnly Property TPN_DEF() As TPN_DEF_col
            Get
                If m_TPN_DEF Is Nothing Then
                    m_TPN_DEF = New TPN_DEF_col
                    m_TPN_DEF.Application = Me
                    m_TPN_DEF.Parent = Me
                    m_TPN_DEF.Refresh()
                End If
                TPN_DEF = m_TPN_DEF
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = TPN_DEF.FindObject(Table, InstID)
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("TPN_DEF_COL")
            TPN_DEF.XMLLoad(e_list, LoadMode)
