...
''' <summary>
'''Реализация документа Текущие значения
''' </summary>
''' <remarks>
'''Данные
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Электроэнергия
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_E As TPLC_E_col


''' <summary>
'''Свойство для доступа к строкам раздела Электроэнергия
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_E() As TPLC_E_col
            Get
                If m_TPLC_E Is Nothing Then
                    m_TPLC_E = New TPLC_E_col
                    m_TPLC_E.Application = Me
                    m_TPLC_E.Parent = Me
                    m_TPLC_E.Refresh()
                End If
                TPLC_E = m_TPLC_E
...
''' <summary>
''' Внутренняя переменная для раздела Мгновенные значения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_M As TPLC_M_col


''' <summary>
'''Свойство для доступа к строкам раздела Мгновенные значения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_M() As TPLC_M_col
            Get
                If m_TPLC_M Is Nothing Then
                    m_TPLC_M = New TPLC_M_col
                    m_TPLC_M.Application = Me
                    m_TPLC_M.Parent = Me
                    m_TPLC_M.Refresh()
                End If
                TPLC_M = m_TPLC_M
...
''' <summary>
''' Внутренняя переменная для раздела Заголовок
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_HEADER As TPLC_HEADER_col


''' <summary>
'''Свойство для доступа к строкам раздела Заголовок
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_HEADER() As TPLC_HEADER_col
            Get
                If m_TPLC_HEADER Is Nothing Then
                    m_TPLC_HEADER = New TPLC_HEADER_col
                    m_TPLC_HEADER.Application = Me
                    m_TPLC_HEADER.Parent = Me
                    m_TPLC_HEADER.Refresh()
                End If
                TPLC_HEADER = m_TPLC_HEADER
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Суточный архив
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_D As TPLC_D_col


''' <summary>
'''Свойство для доступа к строкам раздела Суточный архив
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_D() As TPLC_D_col
            Get
                If m_TPLC_D Is Nothing Then
                    m_TPLC_D = New TPLC_D_col
                    m_TPLC_D.Application = Me
                    m_TPLC_D.Parent = Me
                    m_TPLC_D.Refresh()
                End If
                TPLC_D = m_TPLC_D
...
''' <summary>
''' Внутренняя переменная для раздела Часовые архивы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_H As TPLC_H_col


''' <summary>
'''Свойство для доступа к строкам раздела Часовые архивы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_H() As TPLC_H_col
            Get
                If m_TPLC_H Is Nothing Then
                    m_TPLC_H = New TPLC_H_col
                    m_TPLC_H.Application = Me
                    m_TPLC_H.Parent = Me
                    m_TPLC_H.Refresh()
                End If
                TPLC_H = m_TPLC_H
...
''' <summary>
''' Внутренняя переменная для раздела Пропущенные архивы
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_MISSING As TPLC_MISSING_col


''' <summary>
'''Свойство для доступа к строкам раздела Пропущенные архивы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_MISSING() As TPLC_MISSING_col
            Get
                If m_TPLC_MISSING Is Nothing Then
                    m_TPLC_MISSING = New TPLC_MISSING_col
                    m_TPLC_MISSING.Application = Me
                    m_TPLC_MISSING.Parent = Me
                    m_TPLC_MISSING.Refresh()
                End If
                TPLC_MISSING = m_TPLC_MISSING
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Суммарные показатели
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLC_T As TPLC_T_col


''' <summary>
'''Свойство для доступа к строкам раздела Суммарные показатели
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLC_T() As TPLC_T_col
            Get
                If m_TPLC_T Is Nothing Then
                    m_TPLC_T = New TPLC_T_col
                    m_TPLC_T.Application = Me
                    m_TPLC_T.Parent = Me
                    m_TPLC_T.Refresh()
                End If
                TPLC_T = m_TPLC_T
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = TPLC_HEADER.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLC_M.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLC_H.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLC_D.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLC_T.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLC_MISSING.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLC_E.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("TPLC_HEADER_COL")
            TPLC_HEADER.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLC_M_COL")
            TPLC_M.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLC_H_COL")
            TPLC_H.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLC_D_COL")
            TPLC_D.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLC_T_COL")
            TPLC_T.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLC_MISSING_COL")
            TPLC_MISSING.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLC_E_COL")
            TPLC_E.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            TPLC_HEADER.XMLSave(node, Xdom)
            TPLC_M.XMLSave(node, Xdom)
            TPLC_H.XMLSave(node, Xdom)
            TPLC_D.XMLSave(node, Xdom)
            TPLC_T.XMLSave(node, Xdom)
            TPLC_MISSING.XMLSave(node, Xdom)
            TPLC_E.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    TPLC_HEADER.BatchUpdate
    TPLC_M.BatchUpdate
    TPLC_H.BatchUpdate
    TPLC_D.BatchUpdate
    TPLC_T.BatchUpdate
    TPLC_MISSING.BatchUpdate
    TPLC_E.BatchUpdate
End Sub
    End Class
End Namespace
