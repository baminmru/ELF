...
'''
''' </remarks>
  Private m_TPQ_DEF As TPQ_DEF_col
...
'''
''' </remarks>
        Public ReadOnly Property TPQ_DEF() As TPQ_DEF_col
            Get
                If m_TPQ_DEF Is Nothing Then
                    m_TPQ_DEF = New TPQ_DEF_col
                    m_TPQ_DEF.Application = Me
                    m_TPQ_DEF.Parent = Me
                    m_TPQ_DEF.Refresh()
                End If
                TPQ_DEF = m_TPQ_DEF
...
''' <summary>
''' Внутренняя переменная для раздела Результат обработки
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPQ_result As TPQ_result_col


''' <summary>
'''Свойство для доступа к строкам раздела Результат обработки
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPQ_result() As TPQ_result_col
            Get
                If m_TPQ_result Is Nothing Then
                    m_TPQ_result = New TPQ_result_col
                    m_TPQ_result.Application = Me
                    m_TPQ_result.Parent = Me
                    m_TPQ_result.Refresh()
                End If
                TPQ_result = m_TPQ_result
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = TPQ_DEF.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPQ_result.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("TPQ_DEF_COL")
            TPQ_DEF.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPQ_result_COL")
            TPQ_result.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            TPQ_DEF.XMLSave(node, Xdom)
            TPQ_result.XMLSave(node, Xdom)
        End Sub
