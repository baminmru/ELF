...
''' <summary>
'''Реализация документа t_DEVSCHEMA
''' </summary>
''' <remarks>
'''Схема подключения
''' </remarks>
...
''' <summary>
''' Внутренняя переменная для раздела Схема подключения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLS_INFO As TPLS_INFO_col


''' <summary>
'''Свойство для доступа к строкам раздела Схема подключения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLS_INFO() As TPLS_INFO_col
            Get
                If m_TPLS_INFO Is Nothing Then
                    m_TPLS_INFO = New TPLS_INFO_col
                    m_TPLS_INFO.Application = Me
                    m_TPLS_INFO.Parent = Me
                    m_TPLS_INFO.Refresh()
                End If
                TPLS_INFO = m_TPLS_INFO
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Параметры на схеме
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_TPLS_PARAM As TPLS_PARAM_col


''' <summary>
'''Свойство для доступа к строкам раздела Параметры на схеме
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property TPLS_PARAM() As TPLS_PARAM_col
            Get
                If m_TPLS_PARAM Is Nothing Then
                    m_TPLS_PARAM = New TPLS_PARAM_col
                    m_TPLS_PARAM.Application = Me
                    m_TPLS_PARAM.Parent = Me
                    m_TPLS_PARAM.Refresh()
                End If
                TPLS_PARAM = m_TPLS_PARAM
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = TPLS_INFO.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = TPLS_PARAM.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("TPLS_INFO_COL")
            TPLS_INFO.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("TPLS_PARAM_COL")
            TPLS_PARAM.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            TPLS_INFO.XMLSave(node, Xdom)
            TPLS_PARAM.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    TPLS_INFO.BatchUpdate
    TPLS_PARAM.BatchUpdate
End Sub
    End Class
End Namespace
