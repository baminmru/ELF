...
Public Class viewiu_rcfg_docmodeedit
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.gv.TabIndex = 0
        '
        'viewiu_rcfg_docmodeedit
        '
        Me.Controls.Add (Me.gv)
        Me.name = "viewiu_rcfg_docmodeedit"
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.iu_rcfg_docmode.GetDataTable()
        dt.TableName = "iu_rcfg_docmode"

        ts = New DataGridTableStyle
        ts.MappingName = "iu_rcfg_docmode"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Режим для  создания"
        cs.MappingName = "AddMode"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Режим для редактирования"
        cs.MappingName = "EditMode"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
         cs.ReadOnly = True
        cs.HeaderText = "Можно создавать"
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As bprcfg.bprcfg.iu_rcfg_docmode
        ed = item.FindRowObject("iu_rcfg_docmode", ID)
        Dim gui As Doc_GUIBase

        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
        If gui.ShowPartEditForm("edit", CType(ed, LATIR2.Document.DocRow_Base), mReadOnly ) = True Then
            Dim dt As DataTable
            dt = item.iu_rcfg_docmode.GetDataTable()
            dt.TableName = "iu_rcfg_docmode"
            gv.SetData (dt)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.iu_rcfg_docmode.Add
          Else
              ed = Item.iu_rcfg_docmode.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
        If gui.ShowPartEditForm("edit", CType(ed,LATIR2.Document.DocRow_Base) ) = True Then
            dt = item.iu_rcfg_docmode.GetDataTable()
            dt.TableName = "iu_rcfg_docmode"
            gv.SetData (dt)
        Else
            item.iu_rcfg_docmode.Refresh()
        End If
      End If
    End Sub
