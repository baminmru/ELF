...
''' <summary>
''' Внутренняя переменная для раздела Данные сотрудника
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_iu_u_def As iu_u_def_col


''' <summary>
'''Свойство для доступа к строкам раздела Данные сотрудника
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property iu_u_def() As iu_u_def_col
            Get
                If m_iu_u_def Is Nothing Then
                    m_iu_u_def = New iu_u_def_col
                    m_iu_u_def.Application = Me
                    m_iu_u_def.Parent = Me
                    m_iu_u_def.Refresh()
                End If
                iu_u_def = m_iu_u_def
            End Get
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = iu_u_def.FindObject(Table, InstID)
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("iu_u_def_COL")
            iu_u_def.XMLLoad(e_list, LoadMode)
